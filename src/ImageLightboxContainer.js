...
class ImageLightboxContainer extends React.Component {
  ...
  getData(options) {
    ...
    if (options === "flickr") {
      ...
      const url =
        "https://cors-anywhere.herokuapp.com/https://api.flickr.com/services/feeds/photos_public.gne?tags=puppies&format=json&nojsoncallback=true";
      const images = [];
      fetch(url)
        ...
        .then((jsonData) => {
          ...
          const title =
            jsonData && jsonData.title
              ? `Flickr Public Feed: ${jsonData.title}`
              : "Flickr Public Feed: ";
          for (let i = 0; i < items.length; i++) {
            // convert json into an appropriate 'key/value' object array
  ...
  render() {
    const { lightboxOpen, selectedImage, images, title, firstLoad } =
      this.state;
    return (
      <div id="image-lightbox-container">
        <h1 id="title">{title}</h1>
        {firstLoad && <div id="loadingSpinner">Loading...</div>}
        <ImageGrid images={images} handleClickThumbnail={this.clickThumbnail} />
        <Lightbox
          images={images}
